...
/** Apple-style two-line hamburger → X icon */
function HamburgerIcon({ open }: { open: boolean }) {
  return (
    <div className="flex flex-col justify-center items-center w-6 h-6 gap-[7px]">
      <span
        style={{
          display: "block",
          height: "1.5px",
...
/** The SFL logo SVG — extracted for reuse */
function SFLLogo({ className = "w-10 h-10 sm:w-12 sm:h-12" }: { className?: string }) {
  return (
    <svg
...
export function Navbar() {
  ...
  // Lock body scroll when menu open, close on Escape
  useEffect(() => {
    if (menuOpen) {
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-[60] transition-all duration-500 ${
          menuOpen
            ? "bg-transparent"
            : scrolled
            ? "bg-[#070B14]/40 backdrop-blur-xl border-b border-white/[0.06] shadow-[0_1px_0_0_rgba(255,255,255,0.04)]"
            : "bg-gradient-to-b from-[#070B14]/60 to-transparent backdrop-blur-sm"
        }`}
      >
        <div className="max-w-[1400px] relative mx-auto flex items-center justify-between h-[72px] px-4 md:px-8 lg:px-12">

          {/* ── Logo ── */}
          <Link to="/" className="flex items-center gap-3 group shrink-0 mr-8 lg:mr-12">
            <div className="relative logo-hex">
              ...
              <SFLLogo className="relative z-10 w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20" />
            </div>
            <div className="hidden sm:flex flex-col justify-center gap-[2px]">
              <span className="font-black text-sm tracking-[0.15em] md:tracking-[0.2em] uppercase text-white group-hover:text-sfl-blue transition-colors duration-300 leading-none">
                Sangam Fasteners
              </span>
              <span className="font-black text-sm tracking-[0.15em] md:tracking-[0.2em] uppercase text-white transition-colors duration-300 leading-none">
                Private Limited
              </span>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center justify-center gap-1 flex-1 xl:flex-none xl:absolute xl:left-1/2 xl:top-1/2 xl:-translate-x-1/2 xl:-translate-y-1/2">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`relative px-3.5 py-1.5 text-[13px] font-medium tracking-wide transition-colors duration-200 rounded-md group ${
                  isActive(link.path)
                    ? "text-white"
              ...
              >
                {link.label}
                {/* Underline effect that slides between links */}
                {isActive(link.path) && (
                  <motion.span 
                    layoutId="nav-hover-bar"
                    className="absolute -bottom-1 left-2 right-2 h-[2px] bg-sfl-blue rounded-full"
                    transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                  />
                )}
                {/* Fallback hover effect for non-active links to show underline only when hovered */}
                {!isActive(link.path) && (
                  <span className="absolute -bottom-1 left-2 right-2 h-[2px] bg-white/40 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center rounded-full" />
                )}
              </Link>
            ))}
          </div>
          ...
          <Magnetic>
            <Link
              to="/rfq"
              className="hidden lg:inline-flex items-center gap-2 ml-6 px-4 py-1.5 rounded-full bg-sfl-blue text-white text-[11px] font-bold tracking-widest uppercase hover:bg-sfl-blue/85 hover:shadow-[0_0_20px_rgba(25,148,245,0.4)] transition-all duration-300 group"
            >
              Request Quote
          ...
          <button
            ...
            className="lg:hidden flex items-center justify-center w-10 h-10 rounded-lg text-white transition-colors duration-200 hover:bg-white/[0.06] focus:outline-none"
          >
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[55] bg-[#070B14]/95 backdrop-blur-2xl overflow-y-auto"
          >
            {/* Subtle ambient glow */}
            <div
              ...
              style={{
                background:
                  "radial-gradient(ellipse 60% 50% at 20% 60%, rgba(25,148,245,0.07) 0%, transparent 70%), radial-gradient(ellipse 50% 40% at 80% 30%, rgba(194,154,60,0.04) 0%, transparent 70%)",
              }}
            />

            {/* Content — matches Apple's layout: top bar + link list */}
            <div className="relative z-10 flex flex-col min-h-full">

              {/* Top bar row — just logo + X button */}
              <div className="flex items-center justify-between h-[72px] px-4 md:px-8 border-b border-white/[0.06]">
                <Link
                  to="/"
                ...
                >
                  <div className="relative logo-hex">
                    <SFLLogo className="w-14 h-14 sm:w-16 sm:h-16" />
                  </div>
                  <div className="hidden sm:flex flex-col gap-[2px]">
                    <span className="font-black text-sm tracking-[0.15em] uppercase text-white leading-none">
                      Sangam Fasteners
                    </span>
                    <span className="font-black text-sm tracking-[0.15em] uppercase text-white leading-none">
                      Private Limited
                ...
                <button
                  ...
                  className="flex items-center justify-center w-10 h-10 rounded-lg text-white hover:bg-white/[0.06] transition-colors focus:outline-none"
                >
                  <HamburgerIcon open={true} />
                </button>
              </div>

              {/* Nav link list — Apple-style: large, left-aligned, staggered */}
              <nav className="flex-1 flex flex-col justify-center px-8 sm:px-14 md:px-20 py-12">
                    ...
                    >
                      <Link
                        ...
                        className={`
                          ...
                          ${isActive(link.path) ? "text-sfl-blue" : "text-white/85 hover:text-white"}
                        `}
                      >
                        {link.label}
                        <motion.span
                          className="text-white/20 group-hover:text-white/60 transition-colors"
                          animate={{ x: isActive(link.path) ? 4 : 0 }}
                        >
                          <ArrowRight
                            size={20}
                  ...
                  >
                    <Magnetic>
                      <Link
                        to="/rfq"
                        onClick={() => setMenuOpen(false)}
                        className="group inline-flex items-center gap-3 px-5 py-2.5 bg-sfl-blue text-white rounded-full font-bold text-[12px] tracking-widest uppercase hover:bg-sfl-blue/85 hover:shadow-[0_0_24px_rgba(25,148,245,0.4)] transition-all duration-300"
                      >
                        Request a Quote
                ...
                <motion.div
                  ...
                  className="mt-12 pt-6 border-t border-white/[0.07] flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-10"
                >
                  {/* Contact info */}
                  <div className="text-white/40 text-[11px] tracking-wider space-y-1.5">
                    <p className="text-white/60 font-semibold text-xs uppercase tracking-widest mb-2">Contact</p>
                    <p>+91 98765 43210</p>
                    <p>info@sangamfasteners.com</p>
                  </div>
                  ...
                  <div className="flex items-center gap-5">
                    <a
                      href="#"
                      className="text-white/30 hover:text-sfl-blue transition-colors duration-200"
                      aria-label="LinkedIn"
                    >
                      <Linkedin size={18} />
                    </a>
